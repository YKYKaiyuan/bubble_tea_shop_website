		...
		<div id = "wrapper">
			...
			<div id = "sidebar">
				...
				<?php
					...
					$sql = "SELECT d.D_Name FROM Drink d WHERE NOT EXISTS ((SELECT c.PhoneNumber FROM Customer c) EXCEPT (SELECT o.PhoneNumber From Orders o, Customer c WHERE d.D_Name = o.D_Name AND c.PhoneNumber = o.PhoneNumber))";
					$result = $conn->query($sql);
				...
				<?php
					...
					$sql = "SELECT COUNT(DISTINCT PhoneNumber) AS NUM FROM Orders WHERE PhoneNumber IN (SELECT PhoneNumber FROM Orders WHERE Date = CURRENT_DATE() GROUP BY PhoneNumber HAVING COUNT(Order_ID) >= 1)";
					$result = $conn->query($sql);
